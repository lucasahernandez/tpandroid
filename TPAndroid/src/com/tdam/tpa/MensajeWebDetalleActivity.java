...
public class MensajeWebDetalleActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.detallemensajeweb);

		TextView persona = (TextView) findViewById(R.id.txtPersonaMsgWebDetalle);
		TextView fecha = (TextView) findViewById(R.id.txtFechaMsgWebDetalle);
		TextView body = (TextView) findViewById(R.id.txtBodyMsgWebDetalle);
		ImageView iconoMensajes = (ImageView) findViewById(R.id.iconoMsgWebD);
		// Devuelve el intent con el cual esta actividad fue iniciada
		// (getIntent)
		persona.setText(getIntent().getExtras().getString(
				MensajeWebListaActivity.PARAMETRO_NOMBREMSGWEB));
		fecha.setText(getIntent().getExtras().getString(
				MensajeWebListaActivity.PARAMETRO_FECHAMSGWEB));
		body.setText(getIntent().getExtras().getString(
				MensajeWebListaActivity.PARAMETRO_BODYMSGWEB));
		boolean type=getIntent().getExtras().getBoolean(
				MensajeWebListaActivity.PARAMETRO_TIPOMSGWEB);
		if (type==false) {
			iconoMensajes.setImageResource(R.drawable.identi32sent);
		} else {
			iconoMensajes.setImageResource(R.drawable.identi32);
		}
